...
fp_env_file_miniconda='https://repo.anaconda.com/miniconda/Miniconda3-py312_24.7.1-0-Linux-x86_64.sh'

# Argument(s) default definition(s)
fp_env_tag_default='fp-base'

fp_env_folder_root_default=${LIBS_PATH}libs_conda
fp_env_file_reference_default='virtualenv_python3_%ENV_TAG'
fp_env_folder_libraries_default='virtualenv_python3_%ENV_TAG'

fp_env_file_requirements_default='requirements_fp_system_conda_python3_fp-base.yaml'
...
if [ -f $fp_env_file_requirements ] ; then
	...
	conda create --yes --name $fp_env_folder_libraries numpy scipy pandas matplotlib=3.1.3 rasterio geopandas netCDF4 cython xarray=0.18.0 bottleneck dask pip python=3.7
